...
export class CampaignService {
  ...
  async create(data: CreateCampaignInput) {
    const draftStatus = await prisma.campaignStatus.findUnique({
      where: { name: "Draft" },
    });
    if (!draftStatus)
      throw ApiError.internal(
        'Default campaign status "Draft" not found. Run seed.',
      );
  ...
  async changeStatus(id: string, statusId: string) {
    ...
    const status = await prisma.campaignStatus.findUnique({
      where: { id: statusId },
    });
    if (!status)
      throw ApiError.notFound(`Campaign status "${statusId}" not found`);
