...
export const upsertSocialToken = async (
  ...
  const result = await pool.query(
    `INSERT INTO social_tokens
     ...
     RETURNING *`,
    [userId, platform, encryptedAccessToken, encryptedRefreshToken, tokenExpiresAt, platformUserId, platformUsername, scopes]
  );
...
export const getTokensByUserId = async (userId: number): Promise<SocialToken[]> => {
  const result = await pool.query(
