...
export class TaskService {
  ...
  async moveTask(postId: string, targetStatusId: string) {
    ...
    const targetStatus = await prisma.postStatus.findUnique({
      where: { id: targetStatusId },
    });
    if (!targetStatus)
      throw ApiError.notFound(`Status "${targetStatusId}" not found`);

    if (post.statusId === targetStatusId) {
      throw ApiError.badRequest(
        `Post is already in "${targetStatus.name}" status`,
      );
    }
  ...
  async getBoardView(campaignId: string) {
    ...
    const board = statuses.map((status: any) => ({
      status: {
        id: status.id,
        name: status.name,
        displayOrder: status.displayOrder,
      },
      posts: posts.filter((p: any) => p.statusId === status.id),
    }));
