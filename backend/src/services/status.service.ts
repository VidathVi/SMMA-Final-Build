...
export class StatusService {
  ...
  async createPostStatus(
    ...
    if (existing)
      throw ApiError.conflict(`Post status "${name}" already exists`);

    // Auto-assign displayOrder to end if not provided
    if (displayOrder === undefined) {
      const last = await prisma.postStatus.findFirst({
  ...
  async createCampaignStatus(name: string, description?: string) {
    const existing = await prisma.campaignStatus.findUnique({
      where: { name },
    });
    if (existing)
      throw ApiError.conflict(`Campaign status "${name}" already exists`);
