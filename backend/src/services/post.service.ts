...
export class PostService {
  ...
  async create(data: CreatePostInput) {
    ...
    if (!campaign)
      throw ApiError.notFound(`Campaign "${data.campaignId}" not found`);

    const pendingStatus = await prisma.postStatus.findUnique({
      where: { name: "Pending" },
    });
    if (!pendingStatus)
      throw ApiError.internal(
        'Default post status "Pending" not found. Run seed.',
      );
    ...
    if (data.assignedToId) {
      ...
      if (!user)
        throw ApiError.notFound(`User "${data.assignedToId}" not found`);
    }
