...
const META_REDIRECT_URI =
  process.env.META_REDIRECT_URI || "http://localhost:8080/api/meta/callback";
...
export const metaCallback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.redirect(
      `${process.env.FRONTEND_URL || "http://localhost:3000"}/dashboard/settings?connected=facebook`,
    );
  } catch (error: any) {
  ...
  try {
    ...
    const response = await axios.get(`${GRAPH_API_BASE}/${pageId}/insights`, {
      params: {
        ...
        metric:
          "page_engaged_users,page_impressions,page_fans,page_views_total",
        period: "day",
