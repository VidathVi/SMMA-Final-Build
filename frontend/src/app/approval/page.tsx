...
export default function ApprovalPage() {
  const [projects, setProjects] = useState([
    ...
    {
      name: "Finish Q3 Product Launch Video",
      start: "01/20/2025",
      end: "03/14/2025",
      status: "Not Started",
      priority: "Medium",
    ...
    {
      name: "Work on Summer Collection Instagram Reels",
      start: "04/09/2025",
      end: "03/14/2025",
      status: "Complete",
      priority: "Low",
      assignee: "Barry Clark",
    },
    {
      name: "Design Packaging for New Product Line",
      start: "03/05/2025",
      end: "03/25/2025",
      status: "Not Started",
      priority: "Medium",
    ...
    {
      name: "Schedule Social Media Posts for April",
      start: "03/15/2025",
      end: "03/28/2025",
      status: "Not Started",
      priority: "Low",
    ...
    {
      name: "Revamp Brand Guidelines Document",
      start: "02/18/2025",
      end: "03/23/2025",
      status: "In Progress",
      priority: "Medium",
  ...
  const [sortType, setSortType] = useState<"status" | "priority" | null>(
    "priority",
  );
  ...
  const handleSaveTask = () => {
    ...
    const savedTask = {
      name: newTaskName,
      start: newStartDate.includes("-")
        ? formatDateForDisplay(newStartDate)
        : newStartDate,
      end: newEndDate.includes("-")
      ...
      <main className="flex w-full max-w-[1600px] gap-8 relative p-8">
        <div className="flex-1 flex flex-col gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-xl relative overflow-hidden overflow-x-auto min-h-[80vh]">
            ...
            >
              <div className="flex items-center gap-4 text-xs font-medium text-gray-400 mb-6 hover:text-gray-300"></div>

              <div className="flex items-center gap-3 mb-2">
                <span className="text-4xl text-blue-500 bg-blue-500/10 p-2 rounded-lg">
                  <FiList></FiList>
                </span>
              ...
              <div className="min-w-[1100px]">
                <div className="grid grid-cols-[3fr_1.5fr_1.2fr_1fr_1fr_1fr_1fr_1fr_auto] gap-4 pb-3 border-b border-white/10 font-medium text-gray-400/80 uppercase text-[10px] tracking-wider items-center">
                  <div className="flex items-center gap-2 text-sm normal-case tracking-normal text-gray-400">
                    <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 border border-gray-500 rounded text-gray-400">
                      Task
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-sm normal-case tracking-normal">
                    <FiUser className="w-4 h-4" /> Assignee
                  </div>
                  <div className="flex items-center gap-2 text-sm normal-case tracking-normal">
                    <FiActivity className="w-4 h-4" /> Status
                  </div>
                  <div className="flex items-center gap-2 text-sm normal-case tracking-normal">
                  ...
                  <div className="flex items-center gap-2 text-sm normal-case tracking-normal">
                    <FiTarget className="w-4 h-4" /> Priority
                  </div>
                </div>
                ...
                <div className="flex flex-col">
                  {sortedProjects.map((proj, i) => (
                    <div
                      key={i}
                      onClick={() =>
                        openEditModal(projects.findIndex((p) => p === proj))
                      }
                    ...
                    >
                      <div className="text-white font-medium truncate pr-4 group-hover:text-blue-400 transition-colors">
                        {proj.name}
                      </div>
                      ...
                      <div>
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#3f3f46]/30 text-gray-300 text-xs border border-white/10 whitespace-nowrap">
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${proj.status === "In Progress" ? "bg-blue-400" : proj.status === "Complete" ? "bg-green-400" : "bg-gray-400"}`}
                          ></span>{" "}
                          {proj.status}
                        </span>
                      </div>
                      <div className="text-gray-400">
                        {proj.start || <span className="text-gray-600">-</span>}
                      </div>
                      <div className="text-gray-400">{proj.end}</div>
                      <div>
                        {proj.priority && (
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded text-xs border ${getPriorityColor(proj.priority)}`}
                          >
                            {proj.priority}
                          </span>
                        )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <option
                      value="Not Started"
                      className="bg-[#0A0A3C] text-white"
                    >
                      Not Started
                    </option>
                    <option
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-blue-500 transition-colors appearance-none"
                  >
                    <option value="Low" className="bg-[#0A0A3C] text-white">
                      Low
                    </option>
                    <option value="Medium" className="bg-[#0A0A3C] text-white">
                      Medium
                    </option>
                    <option value="High" className="bg-[#0A0A3C] text-white">
                      High
                    </option>
                  </select>
                </div>
